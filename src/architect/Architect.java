...
public class Architect extends PApplet {
	...
	public void setup() {
		...
		consoleArea = cp5.addTextarea("txt").setPosition(width - 250, 50)
				.setSize(200, 300).setFont(createFont("", 10))
				.setLineHeight(14).setColor(color(0))
	...
	void help() {
		...
		float mod = 20;
		text("H", 10, h);
		text("-  show/hide keyboard commands", 70, h);
		h += mod;
		text("F", 10, h);
		text("-  show/hide frame rate", 70, h);
		h += mod;
		text("S", 10, h);
		text("-  show/hide statistics", 70, h);
		h += mod;
		// text("T",10,h); text("-  show/hide tracing",70,h); h += mod;
		// text("C",10,h); text("-  switch between background colors",70,h); h
		// += mod;
		text("SPACE", 10, h);
		text("-  start/stop animation", 70, h);
		h += mod;
		text("D", 10, h);
		text("-  start/stop population dynamics", 70, h);
		h += mod;
		text("M", 10, h);
		text("-  start/stop mutation /// Adaptive Differential Evolution Global Optimization Algorithm",
				70, h);
		h += mod;
		// text("2",10,h); text("-  switch between 1 and 2 dimensions",70,h); h
		// += mod;
		text("DOWN", 10, h);
		text("-  decrease population size", 70, h);
		h += mod;
		text("UP", 10, h);
		text("-  increase population size", 70, h);
		h += mod;
		text("LEFT", 10, h);
		text("-  decrease generation time", 70, h);
		h += mod;
		text("RIGHT", 10, h);
		text("-  increase generation time", 70, h);
		h += mod;
		text("<", 10, h);
		text("-  decrease trace rate", 70, h);
		h += mod;
		text(">", 10, h);
		text("-  increase trace rate", 70, h);
		h += mod;
		text("CLICK", 10, h);
		text("-  add migrant to population", 70, h);
		h += mod;
	...
	void stats() {
		...
		if (!TWODIMEN && DYNAMICS && TRACING) {
			...
			float t = 0;
			if (N > 0) {
				line(width - 34, height - BASELINE, width - 10, height
						- BASELINE);
			}
			for (int k = N; k > 1; k--) {
				...
				if (mod > 10) {
					if (k < 10) {
						text(k, width - 24, height - t - mod / 2 - BASELINE + 5);
					} else {
						text(k, (float) (width - 27.5), height - t - mod / 2
								- BASELINE + 5);
					}
				}
				t += mod;
				line(width - 34, height - t - BASELINE, width - 10, height - t
						- BASELINE);
			}
		}
	...
	class Individual {
		...
		void displayInd() {
			...
			text(df.format(ax), loc.x, loc.y - 10);
		}
	...
	class Population {
		...
		void replicate() {
			if (pop.size() > 0) {
				...
				pop.add(new Individual(new PVector(newx, newy), ind.hue,
						ind.trace));
		...
		void splitstep() { // called once per frame
			float popBD = (1 / (float) GEN) * (float) N; // population
															// birth-death rate
		...
		void mutate() {
			...
			/*
			 * 
			 * (defn -mutator ;; int int int int [size lowerbound upperbound
			 * maxiter] (println "Population size: " size "LowerBound: "
			 * lowerbound "UpperBound: " upperbound "Maxiter: " maxiter)
			 * (engine/optimized fitness-funct 0.2 0.1 size 10 lowerbound
			 * upperbound maxiter) )
			 */
			// Clojure calls
			...
			for (int i = 0; i < events; i++) {
				int rand = (int) (random(0, pop.size()));
				Individual ind = (Individual) pop.get(rand);
				ax = core.mutator(pop.size(), -100, 100, 2);
		...
		void exclusion() {
			...
			for (int i = 0; i < pop.size(); i++) {
				...
				/*
				 * for (int j = 0 ; j < pop.size(); j++) { if (i != j) {
				 * Individual jnd = (Individual) pop.get(j); float overlap =
				 * ind.r + jnd.r - PVector.dist(ind.loc,jnd.loc); if (overlap >
				 * 0) { ind.reset(); jnd.reset(); } } }
				 */
				// exclude from walls
	...
	// coalescent intervals calculation, returns number of pixels in interval of
	// k lineages
